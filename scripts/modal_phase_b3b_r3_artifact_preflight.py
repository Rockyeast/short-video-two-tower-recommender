#!/usr/bin/env python3
"""Run the B3B-R3 numeric-sidecar artifact preflight on one Modal L4."""
...
RUNNER_COMMIT = "1c2fed6f1f845b2200b77f6d3266dd99876da55c"
REPOSITORY_URL = (
...
app = modal.App("short-video-two-tower-b3b-r3-artifact-preflight")
...
image = (
    ...
    .add_local_file(
        Path(__file__),
        "/opt/modal-wrapper/modal_phase_b3b_r3_artifact_preflight.py",
        copy=True,
    )
...
def _render_markdown(report: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            "# Phase B3B-R3 Numeric Sidecar Artifact Preflight",
            "",
            "Inference-only verification of the frozen final-refit Two-Tower "
            "artifact and numeric-preprocessing sidecar. No recommendation "
            "effectiveness metric was computed.",
            "",
            f"- Runner commit: `{report['runner_commit']}`",
            f"- Wrapper commit: `{report['wrapper_commit']}`",
            f"- Checkpoint SHA256: `{result['checkpoint_sha256']}`",
            (
                "- Numeric sidecar/checkpoint SHA match: "
                f"`{result['numeric_sidecar']['matches_checkpoint']}`"
            ),
            (
...
def run_artifact_preflight() -> dict[str, Any]:
    ...
    result = run(
        ...
        report_json=Path("/tmp/b3b_r3_artifact_preflight.json"),
        device="cuda:0",
    )
    return {
        "phase": "phase-b3b-r3-numeric-sidecar-modal-l4-preflight",
        "runner_commit": RUNNER_COMMIT,
        "artifact_preflight": result,
        "runtime": {
            "gpu_name": gpu_name,
...
def main(
    report_json: str = (
        "reports/phase_b3b_r3/artifact_preflight_modal_l4.json"
    ),
    report_markdown: str = (
        "reports/phase_b3b_r3/artifact_preflight_modal_l4.md"
    ),
) -> None:
