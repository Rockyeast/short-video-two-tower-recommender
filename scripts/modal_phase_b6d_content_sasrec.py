#!/usr/bin/env python3
"""Run the fixed full Content-SASRec experiment on one Modal L4."""
...
OUTPUT_VOLUME_NAME = "kuairec-b6d-content-sasrec-artifacts"
...
app = modal.App("short-video-content-sasrec-l4")
image = (
...
def _runner_inputs_match_commit() -> bool:
    paths = (
        "configs/phase_b6d_content_sasrec_full.yaml",
        "scripts/run_phase_b6a_recbole_sasrec.py",
        "scripts/run_phase_b6c_content_sasrec_smoke.py",
        "scripts/run_phase_b6d_content_sasrec_full.py",
...
def _verify_existing_bundle(files, manifest: dict[str, Any]) -> None:
...
def run_l4(
    *, bundle_sha256: str, input_manifest: dict[str, Any]
    ...
    from scripts.run_phase_b6d_content_sasrec_full import run
    ...
    output_root = Path("/tmp/phase_b6d")
    checkpoints = REMOTE_REPOSITORY / "artifacts/phase_b6d/full"
    report_json = output_root / "content_sasrec_full.json"
    report_markdown = output_root / "content_sasrec_full.md"
    ...
    try:
        ...
        report = run(
            ...
            caption_cache=bundle_root / "caption/caption_embeddings.npz",
    ...
    if (
        report["training_examples"] != 573104
        or len(report["epochs"]) != 5
        or report["validation_counts"]["query_count"] != 6818
        or report["claims"]["small_matrix_accessed"]
        or report["claims"]["temporal_final_accessed"]
    ):
        raise RuntimeError("Full Content-SASRec report contract failed")

    persisted = OUTPUT_MOUNT / "phase-b6d-content-sasrec-v1"
    ...
    return {
        ...
        "runtime": {
            "remote_total_wall_time_s": time.perf_counter() - started,
            "peak_rss_mb": (
...
def main(
    ...
    report_json: str = "reports/phase_b6d/content_sasrec_full_modal_l4.json",
    report_markdown: str = "reports/phase_b6d/content_sasrec_full_modal_l4.md",
) -> None:
    if not _runner_inputs_match_commit():
        raise RuntimeError("Modal runner inputs differ from RUNNER_COMMIT")
    wrapper_commit = _git_output("rev-parse", "HEAD")
    if _git_output("status", "--porcelain"):
        raise RuntimeError("Modal Content-SASRec run requires a clean worktree")
    ...
    result = {
        "phase": "phase-b6d-content-sasrec-modal-l4",
        "runner_commit": RUNNER_COMMIT,
